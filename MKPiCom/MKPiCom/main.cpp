...
void receive_memdump(UartMemDumpPacketInfo dumpInfo) {

  uint32_t currentSegment = 0;
  uint32_t segmentCount = dumpInfo.blockCount;
  
  while (currentSegment < segmentCount) {
    //TODO: msg von Uart auslesen
    
    UartMessage* msg;
    
    UartMemDumpDataBlock block = msg->content.memDumpDataBlock;
    
    printf("MKPiCom :> received block %i of %i", block.block_index, dumpInfo.blockCount);
    
    //TODO: content in file schreiben
    //block.content
...
int main(int argc, const char * argv[]) {
  ...
  cout << "MKPiCom - Serial communication with MKPi\n";
  
  //tty einrichten
  //input/output weiterleitung
  //3x ^C erhalten?
  ...
  while(!leave) {
    ...
    deque<uint8_t>* deque = new std::deque<uint8_t>();
